/*
 * smap.c - Advanced debugger
 *
 * Copyright (C) 2009-2010 jimmikaelkael <jimmikaelkael@wanadoo.fr>
 * Copyright (C) 2009-2010 Eugene Plotnikov
 *
 ...
 */
extern void smap_CopyToFIFO(u16, u32 *, int);
__asm__(
 ".set noreorder\n\t"
 ".set nomacro\n\t"
 ".set noat\n\t"
...
/*
 * smap_CopyFromFIFO: copy datas from FIFO to IOP mem
 */
extern void smap_CopyFromFIFO(u16, u32 *, int);
__asm__(
 ".set noreorder\n\t"
 ".set nomacro\n\t"
 ".set noat\n\t"
...
/*
 * smap_phy_read: read value from the PHY
 */
...
/*
 * smap_phy_write: write value to the PHY
 */
...
/*
 * smap_phy_init: Initialize the PHY
 */
...
/*
 * smap_xmit: low-level, send ethernet frames
 */
...
/*
 * EMAC3 interrupt handler
 */
int smap_emac3_intr(int irq)
{
	USE_SMAP_REGS;
	USE_SMAP_EMAC3_REGS;
	register u32 stat;
...
static int smap_intr_handler(int state)
{
	...
	/* Is it TX interrupt ? */
	if (irq & (SMAP_INTR_TXDNV | SMAP_INTR_TXEND)) {
		smap_tx_intr(irq & (SMAP_INTR_TXDNV | SMAP_INTR_TXEND));
		irq = SPD_REG16(SPD_R_INTR_STAT) & SMAP_INTR_BITMSK;
	}

	/* Is it RX interrupt ? */
	if (irq & (SMAP_INTR_RXDNV | SMAP_INTR_RXEND))
		smap_rx_intr(irq & (SMAP_INTR_RXDNV | SMAP_INTR_RXEND));

	/* Is it EMAC3 interrupt ? */
	else if (irq & SMAP_INTR_EMAC3)
